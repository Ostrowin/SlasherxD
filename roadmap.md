...
**Znalezione 2026-07-20 (zgłoszone przez gracza): w drugim runie nie widać itemów.**
Scena Phasera przeżywa restart (ten sam obiekt), ale wszystkie jej GameObjecty są niszczone. `create()` zerowało trzy z czterech tablic sprite'ów zapełnianych przez `push()` — `pickupSprites` pominięto, więc rosła 120 → 240 → 360. Indeksy 0..119 wskazywały wtedy sprite'y ZNISZCZONE w poprzednim runie: render ustawiał im `visible = true`, ale na ekranie nie było nic. Objaw mylił, bo symulacja działała bez zarzutu — itemy leżały, dawały się zebrać i naliczały bonusy.
Naprawa: brakujące wyzerowanie + **strażnik w trybie dev**, który po każdym `create()` porównuje rozmiar każdej tablicy sprite'ów z jej poolem w symulacji i krzyczy do konsoli przy rozjeździe. Strażnik zweryfikowany przez chwilowe cofnięcie naprawy — złapał błąd i wskazał tablicę po nazwie.
...
- [x] **5h. Prymitywy STATUS i AURA** *(2026-07-20)* — `src/sim/statusConfig.ts`. Status: DoT, spowolnienie, podatność, rozprzestrzenianie, stackowanie (3 sloty na wroga, prealokowane). Aura: status na wrogów, leczenie i wzmacnianie sojuszników, promień. Bonusy z aur przeliczane co tick, więc znikają same po wyjściu z pola. Odblokowuje szczura, wydrę i wilka. 22 asercje, determinizm OK.
      *Przy okazji naprawione: `spawnMobAt` nie czyścił statusów, więc wróg z odzyskanego slotu dziedziczyłby cudzą truciznę.*
- [ ] **5i. Specjalizacje na nowych prymitywach** — następne: **wilk**, **lis**, **niedźwiedź** (użytkownik ma pomysły na motyw)
